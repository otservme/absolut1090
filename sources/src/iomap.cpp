...
bool IOMap::loadMap(Map* map, const std::string& identifier)
{
	...
	if (root_header->minorVersionItems > Items::dwMinorVersion) {
		std::cout << "[Warning - IOMap::loadMap] Este mapa requer sua items.otb atualizada." << std::endl;
	}
	...
	if (type != OTBM_MAP_DATA) {
		setLastErrorString("Incapaz de carregar data node.");
		return false;
	}

	if (!f.getProps(nodeMap, propStream)) {
		setLastErrorString("Incapaz de carregar atributos do mapa.");
		return false;
	}
		...
		switch (attribute) {
			...
			case OTBM_ATTR_EXT_SPAWN_FILE:
				if (!propStream.readString(tmp)) {
					setLastErrorString("Spawn tag invalida.");
					return false;
				}
			...
			case OTBM_ATTR_EXT_HOUSE_FILE:
				if (!propStream.readString(tmp)) {
					setLastErrorString("House tag invalida.");
					return false;
				}
			...
			default:
				setLastErrorString("Cabecalho desconhecido.");
				return false;
		}
	}
	...
	while (nodeMapData != NO_NODE) {
		if (f.getError() != ERROR_NONE) {
			setLastErrorString("Mapa invalido.");
			return false;
		}
	...
	std::cout << "> Mapa carregado em: " << (OTSYS_TIME() - start) / (1000.) << " segundos." << std::endl;
	return true;
}
